// 版权所有(c) HeYuanERP
// 说明：采购订单查询实现（占位实现，后续由基础设施层使用 EF Core 完成数据访问）。
...
/// <summary>
/// 采购订单查询实现。
/// 说明：本实现暂返回空分页结果，用于解耦应用层；数据访问将由基础设施层完善。
/// </summary>
public class POQuery : IPOQuery
{
    public Task<PagedResult<POListItemDto>> QueryAsync(POQueryParamsDto args, CancellationToken ct = default)
    {
        ...
        var result = PagedResult<POListItemDto>.Create(Array.Empty<POListItemDto>(), page, size, total: 0);
