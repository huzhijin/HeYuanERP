...
/// <summary>
/// 库位（Locations）接口：列表/详情/新增/编辑/删除
/// </summary>
[ApiController]
...
{
    private readonly ILocationsService _svc;

    public LocationsController(ILocationsService svc)
    {
        _svc = svc;
    }
    ...
    // 列表
    [HttpGet]
    [RequirePermission("inventory.read")] // B5 将细分为 locations.read
    public async Task<IActionResult> QueryAsync([FromQuery] LocationListQueryDto req, CancellationToken ct)
    {
    ...
    // 新建
    [HttpPost]
    [RequirePermission("inventory.read")] // B5 将细分并改为 locations.create
    public async Task<IActionResult> CreateAsync([FromBody] LocationCreateDto req, CancellationToken ct)
    {
    ...
    [HttpPut("{id}")]
    [RequirePermission("inventory.read")] // B5 将细分并改为 locations.create
    public async Task<IActionResult> UpdateAsync([FromRoute] string id, [FromBody] LocationUpdateDto req, CancellationToken ct)
    {
