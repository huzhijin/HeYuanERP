<template>
  <a-card title="应付账款" :bordered="false">
    <template #extra>
      <a-space>
    ...
    <a-table :data-source="rows" row-key="id" :loading="loading" :pagination="false">
      ...
      <a-table-column title="已付" data-index="paidAmount" width="120" />
...
import { listPayables } from '@/api/modules/finance'
...
async function load(){
  ...
  try{
    ...
    let data:any
    if(status.value) data = await listPayables({ status: status.value })
    else data = await listPayables(p)
